  ...
  if (agentId === "iris") {
    if (context.action === "submit-lead") {
      const payload = {
        ...
        clinic_name: context.clinic_name, city: context.city,
      ...
      if (result.isNew && result.lead?.email && result.lead?.id) {
        ...
        scheduleSequence({
          ...
          lead: {
            ...
            clinicName:  result.lead.clinic_name || result.lead.name,
            city:        result.lead.city || "",
            intakeToken: intakeToken,
          },
        }).catch((err) => console.error("[Iris→Max] submit-lead schedule failed:", err.message));
      }
      return {
        handledBy: "iris",
    ...
    if (context.action === "add-rex-leads") {
      ...
      for (const l of rexLeads) {
        try {
          const clinicName = (l.name || l.clinic_name || "").trim();
          if (!clinicName) { skipped++; continue; }

          // Skip duplicates by clinic_name
