...
export async function runAgent(agentId, message, context = {}) {
  ...
  if (agentId === "dash") {
    ...
    if (context.action === "delete-lead") {
      ...
      return { handledBy: "dash", response: "Lead deleted.", data: { success: true, deleted: result.deleted } };
    }
  ...
  if (agentId === "max") {
    ...
    if (context.action === "delete-lead") {
      const result = await deleteLead(context.leadId);
      return { handledBy: "max", response: "Lead deleted.", data: { success: true, deleted: result.deleted } };
    }
    // No match → fall through to SDK for LLM chat
  }
