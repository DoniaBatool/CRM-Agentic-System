 ...
 * Returns array of matching leads with id, name, organization_name, stage.
 */
export async function searchLeadsByName(name) {
  ...
  const { data, error } = await sb
    .from("agency_leads")
    .select("id, name, organization_name, stage, email, phone, city")
...
export async function getBoardData() {
  ...
  const { data, error } = await sb
    .from("agency_leads")
    .select("id, name, organization_name, email, phone, stage, source, created_at, meeting_datetime, updated_at")
    .order("updated_at", { ascending: false });
