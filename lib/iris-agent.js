...
/**
 * Permanently delete a lead from agency_leads (and its pipeline_history).
 */
export async function deleteLead(leadId) {
  ...
  try {
    await sb.from("pipeline_history").delete().eq("lead_id", leadId);
  } catch (_) {}

  const { error } = await sb.from("agency_leads").delete().eq("id", leadId);
